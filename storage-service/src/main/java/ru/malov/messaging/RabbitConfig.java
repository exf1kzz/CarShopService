...
public class RabbitConfig {
    ...
    @Bean
    Queue storageApprovalQueue() {
        return new Queue(Q_STORAGE_APPROVAL, true);
    }

    @Bean
    Binding b1() {
        return BindingBuilder.bind(storageApprovalQueue()).to(ordersExchange()).with(RK_ORDER_SENT);
    }
}
